...
function simulatePlayer(player, state, Input, delta) {
   ...
   if (distX * distX + distY * distY < (player.radius + state.ball.radius) * (player.radius + state.ball.radius)) {
      ...
      if (speed > 0) {
         ...
         state.ball.xv -= impulse * player.radius * v_collision_norm.x * 1.2 * (player.shift ? 1: 0.4);
         state.ball.yv -= impulse * player.radius * v_collision_norm.y * 1.2 * (player.shift ? 1: 0.4);
